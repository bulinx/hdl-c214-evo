...
private:
  ...
  void publish_odometry(const ros::Time& stamp, const std::string& base_frame_id, const Eigen::Matrix4f& pose) {
    ...
    _pubC214_vicon_pose.publish(odom_trans);
    odom_broadcaster.sendTransform(odom_trans);
    C214_vicon_pose.header.frame_id = "world";
    C214_visual_pose.header.stamp = stamp;
  	//C214_visual_pose.header.frame_id = "world";
    C214_visual_pose.header.frame_id = "world";

    C214_visual_pose.pose.pose.orientation = odom_trans.transform.rotation;//w        w
  	C214_visual_pose.pose.pose.position.x = pose(0, 3);
  	C214_visual_pose.pose.pose.position.y = pose(1, 3);
  	C214_visual_pose.pose.pose.position.z = pose(2, 3);
